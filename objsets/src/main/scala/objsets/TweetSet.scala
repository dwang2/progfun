...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  // should the sum of the tweet counts be the new tweet count when an element is in both lists?
  def union0(that: TweetSet, accu: TweetSet): TweetSet = {
    this.tail.union0(that,accu.incl(this.head))
  }
...
class NonEmptyTrending(elem: Tweet, next: Trending) extends Trending {
  ...
  def - (tw: Tweet): Trending =
    new NonEmptyTrending(tw, next - elem)
  def head: Tweet = elem
...
object GoogleVsApple {
  ...
  def contains(words: List[String]): Tweet => Boolean = {
    def containsF(tweet: Tweet): Boolean = containsF0(tweet,words)
    def containsF0(tweet: Tweet, list: List[String]): Boolean =
      if(list.isEmpty) false
      else if(tweet.text.indexOf(list.head) != -1) true
      else containsF0(tweet,list.tail)
    containsF
  }
  
  val googleTweets: TweetSet = TweetReader.allTweets.filter(contains(google))

  val appleTweets: TweetSet = TweetReader.allTweets.filter(contains(apple))
